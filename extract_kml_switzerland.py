...
def create_kml_for_placemark(placemark):
    ...
    kan_name = datadict['kan_name']
    kan_code = datadict['kan_code']
    ...
    filepath = os.path.join(GEN_ZONES, 'CH-{}-{}.kml'.format(kan_code, kan_name))
...
k = kml.KML()
with open('sources/georef-switzerland-kanton.kml', 'rb') as myfile:
    doc = myfile.read()
    k.from_string(doc)
