...
class Tile(ZoneWithEntries):
    ...
    def getEntryPoints(self, router):
        ...
        for segment in self.segments:
            for nodeA, nodes in router.routing.items():
                for nodeB in nodes:
                    if intersect(segment, (router.nodeLatLon(nodeA), router.nodeLatLon(nodeB))):
                        if isLeft(segment, (segment[0], router.nodeLatLon(nodeA))):
                            coord = Coord(*router.nodeLatLon(nodeA), nodeA)
    ...
    def containsPoint(self, coord):
        contains = (self.lonW<=coord[1]) and (coord[1]<=self.lonE) and (self.latS<=coord[0]) and (coord[0]<=self.latN)
        return contains
